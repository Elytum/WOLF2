...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2015/03/04 10:01:05 by achazal           #+#    #+#             */
/*   Updated: 2015/03/04 10:01:06 by achazal          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void			ft_drawskybox2(t_env *e, int sc, int img)
{
...
void			ft_drawskybox(t_env *e, int ox, int iy)
{
	...
	while (y < HEIGHT)
	{
		...
		while (x < WIDTH)
		{
			...
			while (ix < 0)
				ix += e->background.width;
			ft_drawskybox2(e, y * e->size_line + x * (e->bpp / 8),
				iy * e->background.size_line + ix *
				(e->background.bpp / 8));
			x++;
			ix++;
		}
